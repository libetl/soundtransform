...
public class SlowdownSoundTransformation extends
    ...
    public SlowdownSoundTransformation (FourierTransformHelper helper1,
            final int threshold, final float factor) {
        super (helper1);
        this.factor = factor;
    ...
    public Sound initSound (final Sound input) {
        ...
        this.sound = new Sound (newdata, input.getNbBytesPerSample (),
    ...
    public Spectrum transformFrequencies (final Spectrum fs, final int offset) {
        ...
        final int padding = (int) Math.floor (this.writeIfGreaterEqThan1
                + remaining);
        final int loops = (int) (this.factor + padding - 1);
        this.additionalFrames += loops;
        for (int p = 0 ; p < loops ; p++) {
            complexArray = fastFourierTransformer.transform (complexArray,
                    TransformType.INVERSE);

            for (int j = 0 ; j < fs.getSampleRate () ; j++) {
                if (offset + p * fs.getSampleRate () + j < this.sound
                        .getSamples ().length
                        && this.sound.getSamples () [offset + p
                                * fs.getSampleRate () + j] == 0) {
                    this.sound.getSamples () [offset + p * fs.getSampleRate ()
                            + j] = (long) Math.floor (complexArray [j]
                            .getReal ());
                }
            }
        }
