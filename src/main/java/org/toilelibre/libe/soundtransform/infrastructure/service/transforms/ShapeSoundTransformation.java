...
public class ShapeSoundTransformation implements SoundTransformation,
        LogAware<ShapeSoundTransformation> {
    ...
    public Sound transform (final Sound sound) throws SoundTransformException {
        ...
        return this.transform (sound.getSamples ().length, threshold,
                sound.getNbBytesPerSample (), sound.getSampleRate (),
                channelNum);
    }

    public Sound transform (int length, int threshold, int nbBytesPerSample,
            int sampleRate, int channelNum) throws SoundTransformException {
        final Sound builtSound = new Sound (new long [length],
                nbBytesPerSample, sampleRate, channelNum);
        double lastFreq = freqs [0];
        int lastBegining = 0;
        int countZeros = 0;
        for (int i = 0 ; i < freqs.length ; i++) {
            final float lengthInSeconds = (i - lastBegining < 1 ? freqs [i]
                    * threshold : (i - 1 - lastBegining) * threshold * 1.0f)
                    / sampleRate;
            ...
            if (i == freqs.length - 1 || freqChanged
                    && (lastFreq == 0 || freqs [i] == 0 && countZeros >= 3)) {
                countZeros = 0;
                Note note = this.findNote (lastFreq, sampleRate, i,
                        lastBegining);
                this.soundAppender.appendNote (builtSound, note, lastFreq,
                        threshold * lastBegining, channelNum, lengthInSeconds);
                lastBegining = i;
                lastFreq = freqs [i];
            }
        }
    ...
    private Note findNote (double lastFreq, int sampleRate, int i,
            int lastBegining) throws SoundTransformException {
        Note note = silence;
        if (lastFreq > 50 && Math.abs (sampleRate - lastFreq) > 100) {
            this.log (new LogEvent (LogLevel.VERBOSE, "Note (" + lastFreq
                    + "Hz) between " + lastBegining + "/" + freqs.length
                    + " and " + i + "/" + freqs.length));
            if (!this.pack.containsKey (this.instrument)) {
                throw new SoundTransformException (
                        ShapeSoundTransformationErrorCode.NOT_AN_INSTRUMENT,
                        new NullPointerException (), this.instrument);
            }
            note = this.pack.get (this.instrument).getNearestNote (
                    (int) lastFreq);
        }
        return note;
    }
    ...
    private int [] getLoudestFreqs (Sound sound, int threshold) {
        final PeakFindWithHPSSoundTransformation peak = $.create (
                PeakFindWithHPSSoundTransformation.class, threshold, -1);
