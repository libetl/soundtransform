...
import org.toilelibre.libe.soundtransform.infrastructure.service.audioformat.android.HasStreamInfo;
...
public class AndroidPlaySoundProcessor extends AbstractLogAware<AndroidPlaySoundProcessor> implements PlaySoundProcessor {
    ...
    public Object play (final InputStream ais) throws PlaySoundException {
        if (!(ais instanceof HasStreamInfo)) {
            throw new PlaySoundException (new SoundTransformException (PlaySoundErrorCode.COULD_NOT_PLAY_SOUND, new ClassCastException ("Could not cas InputStream as a HasStreamInfo class")));

        }
        final HasStreamInfo is = (HasStreamInfo) ais;
