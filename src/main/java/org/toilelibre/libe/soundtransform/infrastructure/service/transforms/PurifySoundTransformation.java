...
public class PurifySoundTransformation extends
        SimpleFrequencySoundTransformation {

    public PurifySoundTransformation (FourierTransformHelper helper1) {
        super (helper1);
    }
    ...
    public Spectrum transformFrequencies (final Spectrum fs, final int offset,
        ...
        for (int j = 0 ; j < length ; j++) {
            final double tmp = Math.sqrt (Math.pow (
                    fs.getState () [j].getReal (), 2)
                    + Math.pow (fs.getState () [j].getImaginary (), 2));
        ...
        for (int j = 0 ; j < powOf2NearestLength ; j++) {
            newAmpl [j] = fs.getState () [j].multiply (Math.exp (-Math.pow (j
                    - max, 2) / 100));
        }
