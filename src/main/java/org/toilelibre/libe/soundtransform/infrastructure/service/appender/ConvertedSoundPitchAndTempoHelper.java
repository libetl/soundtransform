...
public class ConvertedSoundPitchAndTempoHelper implements
        SoundPitchAndTempoHelper {

    @Override
    public Sound pitchAndSetLength (final Sound sound, final float percent,
        ...
        final double factor = sound.getSamples ().length == 0 ? 0 : 1.0
                * lengthInSeconds * sound.getSampleRate ()
                / result.getSamples ().length;
        if (factor == 0) {
            return result;
        } else if (factor < 0.98 || factor > 1.02) {
            if (factor < 0.98) {
                final SpeedUpSoundTransformation speedup = $.create (
                        SpeedUpSoundTransformation.class, 100,
                        (float) (1 / factor));
                result = speedup.transform (result);

            } else if (factor > 1.02) {
                final SlowdownSoundTransformation slowdown = $.create (
                        SlowdownSoundTransformation.class, 100, (float) factor);
