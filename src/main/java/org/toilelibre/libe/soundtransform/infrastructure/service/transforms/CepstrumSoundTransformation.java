...
public class CepstrumSoundTransformation extends
    ...
    public CepstrumSoundTransformation (FourierTransformHelper helper1,
            Spectrum2CepstrumHelper helper2, SpectrumHelper helper3) {
        super (helper1);
        this.threshold = 100;
    ...
    public int getWindowLength (final double freqmax) {
        if (this.length < CepstrumSoundTransformation.shortSoundLength) {
            return (int) Math.pow (2,
                    Math.ceil (Math.log (this.length) / Math.log (2)));
        }
        return (int) Math
                .pow (2, Math.ceil (Math.log (freqmax) / Math.log (2)));
    }
    ...
    public Spectrum transformFrequencies (final Spectrum fs) {
        ...
        final Spectrum fscep = this.spectrum2CepstrumHelper
                .spectrumToCepstrum (fs);

        this.loudestfreqs [this.index] = this.spectrumHelper.getMaxIndex (
                fscep, 0, fs.getSampleRate ());
        this.index++;
