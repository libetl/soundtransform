...
public class ConvertedSoundAppender implements SoundAppender {
     ...
     */
    @Override
    public void append (final Sound origin, final int usedarraylength,
            final Sound... otherSounds) {
     ...
     */
    @Override
    public int append (final Sound origin, final int usedarraylength,
            final Sound otherSound) {
        final Sound resultBeforeResize = this.changeNbBytesPerSample (
                otherSound, origin.getNbBytesPerSample ());
        final Sound resultBeforeCopy = this.resizeToSampleRate (
                resultBeforeResize, origin.getSampleRate ());
        final int lastIndex = Math.min (origin.getSamples ().length,
                usedarraylength + resultBeforeCopy.getSamples ().length);
        System.arraycopy (resultBeforeCopy.getSamples (), 0,
                origin.getSamples (), usedarraylength, lastIndex
                        - usedarraylength);
        return lastIndex;
    }
     ...
     */
    @Override
    public Sound changeNbBytesPerSample (final Sound sound,
            final int newNbBytesPerSample) {
        final long [] newsamples = new long [sound.getSamples ().length];
        final long oldMax = (long) (Math
                .pow (256, sound.getNbBytesPerSample ()) / 2);
        ...
        return new Sound (newsamples, newNbBytesPerSample,
                sound.getSampleRate (), sound.getChannelNum ());
    }
    ...
    public Sound downsampleWithRatio (final Sound sound, final float ratio) {
        ...
        int indexResult = 0;
        final long [] result = new long [(int) Math
                .ceil (sound.getSamples ().length / ratio)];
        ...
        return new Sound (result, sound.getNbBytesPerSample (),
    ...
    private Sound upsampleWithRatio (final Sound sound, final float ratio) {
        ...
        int indexResult = 0;
        final long [] result = new long [(int) Math
                .ceil (sound.getSamples ().length * (ratio + 1))];
        for (int i = 0 ; i < sound.getSamples ().length ; i++) {
        ...
        return new Sound (indexResult == 0 ? new long [0] : Arrays.copyOfRange (
                result, 0, indexResult - 1), sound.getNbBytesPerSample (),
                (int) (sound.getSampleRate () * ratio), sound.getChannelNum ());
    }

    @Override
    public void appendNote (Sound sound, Note note, double lastFreq,
            int indexInSound, int channelNum, float lengthInSeconds) {

        if (lengthInSeconds < 0.6) {
            final Sound sustain = note.getSustain ((int) lastFreq, channelNum,
                    lengthInSeconds * 2);
            this.append (sound, indexInSound, sustain);
        } else {
            final Sound attack = note.getAttack ((int) lastFreq, channelNum,
                    lengthInSeconds);
            final Sound decay = note.getDecay ((int) lastFreq, channelNum,
                    lengthInSeconds);
            final Sound sustain = note.getSustain ((int) lastFreq, channelNum,
                    lengthInSeconds);
            final Sound release = note.getRelease ((int) lastFreq, channelNum,
                    lengthInSeconds);
