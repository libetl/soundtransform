...
public class AddNoteService {
    ...
    enum AddNoteErrorCode implements ErrorCode {
        FILE_NOT_FOUND ("%1s not found"), COULD_NOT_BE_PARSED (
                "%1s could not be parsed as an ADSR note"), NOT_READABLE (
                "%1s could not be read"), NOT_SUPPORTED (
    ...
    public void addNote (final Range range, final String fileName)
            throws SoundTransformException {
        final ClassLoader classLoader = Thread.currentThread ()
                .getContextClassLoader ();
        try {
            final java.net.URL completeURL = classLoader.getResource ("notes/"
                    + fileName);
            if (completeURL == null) {
                throw new SoundTransformException (
                        AddNoteErrorCode.FILE_NOT_FOUND,
                        new FileNotFoundException (fileName), fileName);
            }
            ...
            final Note n = $.create (Sound2NoteService.class).convert (
                    fileName,
                    $.create (TransformInputStreamService.class)
                            .fromInputStream (
            ...
            throw new SoundTransformException (
                    AddNoteErrorCode.COULD_NOT_BE_PARSED, e, fileName);
        }

    }

    public void addNote (final Range range, final String fileName,
            final int frequency) throws SoundTransformException {
        final ClassLoader classLoader = Thread.currentThread ()
                .getContextClassLoader ();
        try {
            final java.net.URL completeURL = classLoader.getResource ("notes/"
    ...
    public void addNotes (final Range range, final String... fileNames)
            throws SoundTransformException {
