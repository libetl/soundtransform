...
public class SimpleNote implements Note {
	...
	private float getRatio (final Sound [] subsound) {
		...
		final float lengthOfSound = 1.0f * this.attack [0].getSamples ().length / this.attack [0].getSampleRate () + 1.0f * this.decay [0].getSamples ().length / this.decay [0].getSampleRate ()
		        + 1.0f * this.sustain [0].getSamples ().length / this.sustain [0].getSampleRate () + 1.0f * this.release [0].getSamples ().length / this.release [0].getSampleRate ();
