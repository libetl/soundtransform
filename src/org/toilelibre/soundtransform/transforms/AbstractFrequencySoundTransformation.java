...
public abstract class AbstractFrequencySoundTransformation implements
		SoundTransformation, LogAware {
	...
	protected abstract FrequenciesState transformFrequencies(
			FrequenciesState fs, int offset, int powOf2NearestLength,
			int length, double maxfrequency);
	...
	public Sound transform(Sound sound) {
		...
		int maxlength = (int) Math.pow(2,
				Math.ceil(Math.log(freqmax) / Math.log(2)));
		...
		for (int i = 0; i < data.length; i += threshold) {
			...
			Complex[] complexArray = fastFourierTransformer.transform(
					transformeddata, TransformType.FORWARD);
			FrequenciesState fs = new FrequenciesState(complexArray);
			FrequenciesState result = this.transformFrequencies(fs, i,
					maxlength, length, freqmax);
			complexArray = fastFourierTransformer.transform(result.getState(),
			...
			for (int j = 0; j < freqmax; j++) {
				if (i + j + k < newdata.length && newdata[i + j + k] == 0) {
					newdata[i + j + k] = (long) Math.floor(complexArray[j]
							.getReal());
				}
			}
		}
