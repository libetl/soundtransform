...
public class TransformSound implements LogAware {
	...
	public AudioInputStream transformAudioStream(AudioInputStream ais,
		...
		for (SoundTransformation st : sts) {
			for (int i = 0; i < input.length; i++) {
				this.notifyAll("Transform " + (transformNumber + 1) + "/"
						+ sts.length + " (" + st.getClass().getSimpleName()
						+ "), channel " + (i + 1) + "/" + input.length);
	...
	private Sound[] fromInputStream(InputStream ais, int channels,
			long frameLength, int sampleSize, double sampleRate,
			boolean bigEndian, boolean pcmSigned) throws IOException {
		...
		for (int channel = 0; channel < channels; channel++) {
			ret[channel] = new Sound(new long[(int) frameLength], sampleSize,
					(int) sampleRate);
		}
		for (int position = 0; position < frameLength; position++) {
			byte[] frame = new byte[sampleSize * channels];
			ais.read(frame);
			this.byteArrayToFrame(frame, ret, position, bigEndian, pcmSigned,
					neutral);
		}
	...
	private Sound[] fromInputStream(AudioInputStream ais) throws IOException {
		...
		return this.fromInputStream(ais, channels, frameLength, sampleSize,
				sampleRate, bigEndian, pcmSigned);
	}

	private void byteArrayToFrame(byte[] frame, Sound[] sound, int position,
			boolean bigEndian, boolean pcmSigned, long neutral) {
		...
		for (int j = 0; j < frame.length; j++) {
			...
			int currentChannel = (!bigEndian ? j
			...
			if (fromIndex <= toIndex) {
				...
				value[currentChannel] += (frame[cursor] + (pcmSigned ? -Byte.MIN_VALUE : 0))
						* Math.pow(256, numByte);
				// this.log(new LogEvent (LogLevel.PARANOIAC,
				// "Building Sample #" + position + ", channel " +
				// currentChannel + ", numByte : " + numByte + ", value : " +
				// String.format("%.0f", oldValue) + " + ((" + (frame [cursor] +
				// " +  " + (pcmSigned ? -Byte.MIN_VALUE : 0)) + ") * 256^" +
				// numByte + ") = " + String.format("%.0f", value
				// [currentChannel])));
			}

		}
		// this.log(new LogEvent (LogLevel.PARANOIAC, "Sample #" + position +
	...
	protected Sound[] byteArrayToFrames(byte[] byteArray, int channels,
			long frameLength, int sampleSize, double sampleRate,
			boolean bigEndian, boolean pcmSigned) throws IOException {
		this.notifyAll("[Test] byteArray -> ByteArrayInputStream");

		ByteArrayInputStream bais = new ByteArrayInputStream(byteArray);
		return this.fromInputStream(bais, channels, frameLength, sampleSize,
				sampleRate, bigEndian, pcmSigned);
	}

	protected byte[] framesToByteArray(Sound[] channels, int sampleSize,
			boolean bigEndian, boolean pcmSigned) {
		int length = channels.length * sampleSize
				* channels[0].getSamples().length;
		byte[] data = new byte[length];
		// this.log(new LogEvent (LogLevel.PARANOIAC, "SampleSize : " +
		// sampleSize + ", channelsLength : " + channels.length));
		...
		for (int i = 0; i < data.length; i++) {
			...
			if (numByte == 0) {
				value = channels[currentChannel].getSamples()[currentFrame]
						+ neutral;
				// this.log(new LogEvent (LogLevel.PARANOIAC, "Sample #" +
				// currentFrame + ", channel : " + currentChannel + ", value : "
				// + String.format("%.0f", value)));
			}
			dividedValue = value / 256;
			byteValueSigned = (byte) (value + (pcmSigned ? Byte.MIN_VALUE : 0));
			// this.log(new LogEvent (LogLevel.PARANOIAC, "Sample #" +
			// currentFrame + ", channel : " + currentChannel + ", numByte : " +
			// numByte + ", byteValue : " + byteValueSigned));
	...
	private AudioInputStream toStream(Sound[] channels, AudioFormat audioFormat) {
		...
		int length = audioFormat.getFrameSize()
				* channels[0].getSamples().length;
		byte[] data = this.framesToByteArray(channels,
				audioFormat.getFrameSize() / channels.length,
				audioFormat.isBigEndian(),
				audioFormat.getEncoding() == Encoding.PCM_SIGNED);
		this.notifyAll("Creating output file");
		// now save the file
		ByteArrayInputStream bais = new ByteArrayInputStream(data);
		return new AudioInputStream(bais, audioFormat, length
				/ audioFormat.getFrameSize());
	}

	public void transformFile(File fOrigin, File fDest,
			SoundTransformation... sts) throws UnsupportedAudioFileException,
			IOException {
		File file = fOrigin;
