...
public class ShapeTest {
    ...
    @Test
    public void testAppendSoundsWithDifferentNbBytes ()
            throws SoundTransformException {
        final ClassLoader classLoader = Thread.currentThread ()
                .getContextClassLoader ();
        final File input1 = new File (classLoader.getResource (
                "notes/Piano2-D.wav").getFile ());
        final File input2 = new File (classLoader.getResource (
                "notes/g-piano3.wav").getFile ());
        final Sound [] s1 = $.create (TransformSoundService.class)
                .fromInputStream (
                        $.create (ConvertAudioFileService.class).callConverter (
                                input1));
        final Sound [] s2 = $.create (TransformSoundService.class)
                .fromInputStream (
                        $.create (ConvertAudioFileService.class).callConverter (
                                input2));
    ...
    public void testShapeASimplePianoNoteAsAChordNote ()
        ...
        try {
            ...
            final File input = new File (classLoader.getResource (
                    "notes/Piano5-G.wav").getFile ());
            final File output = new File (new File (classLoader.getResource (
                    "before.wav").getFile ()).getParent ()
                    + "/after.wav");
            final InputStream outputStream = $.create (
                    TransformSoundService.class,
                    new PrintlnTransformObserver ()).transformAudioStream (
                    $.create (ConvertAudioFileService.class).callConverter (
                            input),
                    new ShapeSoundTransformation (Library.defaultPack,
                            "chord_piano"));

            AudioSystem.write ((AudioInputStream) outputStream,
                    AudioFileFormat.Type.WAVE, output);

            final int frequency = $
                    .create (Sound2NoteService.class)
                    .convert (
                            "output chord_note",
                            $.create (TransformSoundService.class,
                                    new PrintlnTransformObserver ())
                                    .fromInputStream (
                                            $.create (
                                                    ConvertAudioFileService.class)
                                                    .callConverter (output)))
                    .getFrequency ();
            System.out
                    .println ("Output chord note should be around 387Hz, but is "
                            + frequency + "Hz");
    ...
    public void testShapeASimplePianoNoteAsAChordNoteSameFrequency ()
        ...
        try {
            ...
            final int frequency = $
                    ...
                    .getFrequency ();
            System.out
                    .println ("Output chord note should be around 332Hz, but is "
                            + frequency + "Hz");
        } catch (final IOException e) {
            e.printStackTrace ();
        }
