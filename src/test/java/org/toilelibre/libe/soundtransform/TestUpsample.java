...
public class TestUpsample {
    ...
    public void testTransform11025Hz2BitsMonoSoundInto44100Hz2BitsMonoSound ()
            throws SoundTransformException, IOException {
        ...
        final File output = new File (new File (classLoader.getResource (
                ...
                + "/after.wav");
        final Sound [] inputSounds = $.create (TransformSoundService.class)
                .fromInputStream (
        ...
        for (int i = 0 ; i < inputSounds.length ; i++) {
            Sound tmp = new ConvertedSoundAppender ().changeNbBytesPerSample (
                    inputSounds [i], 2);
        ...
        final InputStream ais = $.create (TransformSoundService.class)
                .toStream (
                        outputSounds,
                        new InputStreamInfo (outputSounds.length,
                                outputSounds [0].getSamples ().length, 2,
                                44100, false, true));
        AudioSystem.write ((AudioInputStream) ais, AudioFileFormat.Type.WAVE,
                output);
    ...
    public void testTransform8363Hz1BitMonoSoundInto44100Hz2BitsMonoSound ()
        ...
        for (int i = 0 ; i < inputSounds.length ; i++) {
            ...
            // [i], 2);
            final Sound tmp = new ConvertedSoundAppender ().resizeToSampleRate (
                    inputSounds [i], 44100);
            outputSounds [i] = tmp;
        }
